...
namespace RapportFraStedet.Models
{
    ...
    public class RepositorySelections
    {
        private DatabaseFormsEntities db = new DatabaseFormsEntities();
        public IQueryable<Selection> GetSelections(int id)
        {
            return db.Selections.Where(m => m.FieldId == id).OrderBy(a => a.SelectionOrder);
        }
        public Selection Get(int id)
        {
            return db.Selections.Single(m => m.SelectionId == id);
        }
        //
